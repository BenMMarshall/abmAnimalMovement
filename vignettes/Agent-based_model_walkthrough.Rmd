...
For example, space use has been tied directly to body size connected via the energy requirements for animals <!-- likely need a little more detail --> [@noonan_effects_2020], and movement has provided incredible insights into behaviour <!-- Studd paper on lynx -->.
...
In the past decade the volume of animal data has exploded, and the infrastructure for collating that data improved drastically [@kays_movebank_2022].
There are stand out examples of novel methods and exceptionally detailed insights into animal behaviour using new bio-logging technology.
...
Studies tend to focus on larger, less vulnerable species [@crane_lots_2021; @tam_quantifying_2021], but as the benefits of detailed movement data grow we can expect greater demand for tracking species of conservation concern.
...
There are a number of existing approaches for simulating animal movement: <!-- ctmm, moveHMM, that recent paper, the C based one for rivers-->, all with different aspects of animal movement included.
Some efforts can be quite specialised to a particular scenario, such as migratory behaviour [@bennett_modelling_2006]; while other target one aspect of movement such as home range [@VanMoorter2009].
