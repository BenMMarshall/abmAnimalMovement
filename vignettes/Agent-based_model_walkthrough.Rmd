...
For most applications a landscape would be known **a priori**, but for this demonstration and testing of methods we will use a selection of random generated landscape rasters.
...
To achieve this we assign cells with values between 0.6 and 0.3 a higher foraging quality [Figure \@ref(fig:landscapeLayers)], and reduce the shelter quality in all cells with values less than 0.3 (i.e., to prevent sheltering in completely unsuitable locations).
...
-   For the sett, we pass a single set of coordinates (`shelterLocations`) and define the size of the shelter site as 5 m (`shelterSize`).
...
landscapeLayersList$movement[] <- 0
...
In the same fashion as the badger, we can supply the simulation with predefined shelter sites, either randomly based on the shelter raster or manually defined (e.g., if shelter sites were known *a priori* discovered via the capture and tagged of animals).
...
KINGCOBRA_shelterLocs <- data.frame(
  "x" = c(1020, 1250, 980, 1000, 800, 700, 1200, 1400),
  "y" = c(1020, 1450, 900, 1000, 800, 700, 1200, 1400)
)
...
for(species in vecSpecies){
  ...
  simResultsList[[species]] <- abm_simulate(
    ...
    movementMatrix = if(length(ls(pattern = paste0("^", species, ".*movementMatrix"))) > 0){
      get(ls(pattern = paste0("^", sp, ".*movementMatrix")))
    } else {
      # DEFAULT HERE
