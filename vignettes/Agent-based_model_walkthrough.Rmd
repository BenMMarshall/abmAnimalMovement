...
BADGER_k_step <- c(1.5*60, 1.25*60, 0.25*60)
BADGER_s_step <- c(0.25, 0.25, 0.5)
...
BADGER_rest_Cycle <- c(0.40, 0, 24, 24)

## multiple cycle additions
c0 <- c(0.01, 0, 24, 24*28) # seasonal
...
VULTURE_k_step <- c(1.25*60, 2*60, 1.5*60)
VULTURE_s_step <- c(0.5, 1.2, 1)
...
VULTURE_rest_Cycle <- c(0.42, 0, 24, 24)

## multiple cycle additions
c0 <- c(0.02, 0, 24, 24*28) # seasonal
...
KINGCOBRA_destinationRange <- c(100, 8)
...
for(species in vecSpecies){
  ...
  tempPlot <- abmMapPlot(simRes,
                         ...
                         plotOptions = FALSE) +
    ggplot2::labs(title = 
                    ifelse(species == "BADGER", "A-  Badger",
                           ifelse(species == "VULTURE", "B - Vulture",
                                  "C - King Cobra"))
    ) +
...
for(species in vecSpecies){
  ...
  zoom_breaks <- function(x){
    if(max(x) > 5){
      seq(0, simSteps/60/24, simSteps/60/24/7)
    }else{
      seq(0, 4, 1)
    }
  }
  ...
  tempPlot <- cycleData %>%
    ...
    ggplot2::labs(y = ifelse(species == "BADGER",
                  ...
                  x = "Days",
                  title = ifelse(
                    species == "BADGER", "A - Badger",
                    ifelse(species == "VULTURE", "B - Vulture", "C - King Cobra")
                  )) +
