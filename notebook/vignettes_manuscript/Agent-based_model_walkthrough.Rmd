...
```{r palettePreview, echo=FALSE, eval=TRUE, fig.align='center', fig.width=7, fig.height=2.5, fig.cap="Universal colour key. Name assigned to the colour in bold, and the hex code in italics below."}
names(palette) <- c("KING\nCOBRA", "VULTURE", "BADGER", "State 2", "State 1", "State 0")
data.frame(
  names = factor(names(palette), levels = c(
    "State 0", "State 1", "State 2", "BADGER", "VULTURE", "KING\nCOBRA")),
  hexcodes = palette
) %>% 
  ggplot() +
  geom_col(
