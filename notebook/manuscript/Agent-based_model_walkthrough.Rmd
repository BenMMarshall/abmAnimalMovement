...
Where `destinationRange` provides the shape (k) and scale ($\theta$) for the Gamma distribution describing distance of possible foraging locations, and `destinationDirection` provides the mean ($\mu$) and concentration ($\kappa$) the Von Mises distribution describing the angle at which those locations can fall in relation to the previous direction of movement (Figure \@ref(fig:BADGERmoveCharFigure)).
...
The example badger, vulture, and king cobra data simulated for this manuscript are available on Zenodo: "Simulated data from abmAnimalMovement: An R package for simulating animal movement using an agent-based model" https://doi.org/10.5281/zenodo.6992496. Files include:
...
A minimally dependent example of the simulation function can be found at https://doi.org/10.5281/zenodo.6992495 alongside pre-run examples of the output simulation data.
