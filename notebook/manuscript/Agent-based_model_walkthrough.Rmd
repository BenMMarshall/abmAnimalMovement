...
As the biodiversity crisis deepens, new refined methods of mitigating human impacts on animals are needed.
Developing mitigation strategies is aided by a strong understanding of animals lives and needs.
How animals' move offers a window into their decision-making process [@Bastille-Rousseau2017; @sridhar_geometry_2021], as well as how they prioritise resources [@prange_influences_2004; @christy_experimental_2017; @vogt_suitability_2018], avoid threats [@loveridge_landscape_2017], and react to anthropogenic modification of the environment [@Tucker2018; @tucker_large_2019; @arrondo_invisible_2018].
...
For example, space use has been tied directly to body size via the energy requirements for animals <!-- likely need a little more detail --> [@noonan_effects_2020], and movement has provided incredible insights into behaviour [@studd_purrfect_2021].

<!-- -   therefore we need studies to be more ethical and cost effective -->
<!-- - key in conservation -- webb et al on ethics animal stuff? -->

Information gathered from animal movement studies is only as robust as the methods and analyses applied.
In the past decade, the volume of animal movement data has increased [@joo_recent_2022; @wild_internet_2022], and the infrastructure for collating these data has improved drastically [@kays_movebank_2022].
There are stand out examples of novel methods [@saunders_radio-tracking_2022], and exceptionally detailed insights into animal behaviour using new bio-logging technology [@studd_purrfect_2021].
...
Whereas lab studies can more feasibly be repeated (although such replications are expensive), movement studies conducted on free-ranging wild animals are difficult to repeat while satisfactorily meeting the conditions of the true replication [@fraser_role_2020].
<!-- likely need to add something defining true replication here if not paired with multiverse context review -->
It is hard to justify placing bio-telemetry devices on more animals to repeat a study, especially for species of conservation concern.
...
With the limited scope for repeating wild movement studies, and the desire to maximise study-to-benefit ratio for sensitive species, it is imperative that researchers maximise the quality of the initial studies.
...
As the complexity of the research environment increases, so must the analytical approach to account for the uncontrollable --but potentially extraneous-- variables.
...
The suite of tools for simulating aspects of ecological systems (e.g., landscapes [@Sciaini2018],  genetics [@petr_slendr_2022], conservation choices [@gupta_reserve_2019], ecological community comparisons [@guerracastro_ssp_2021], resource selection [@street_solving_2021], home range size [@Silva_movedesign_2023; @ines_silva_2023_7963190]) is growing, and some are explicitly focusing on aiding study design.
...
Once validated with real-world data, simulated explorations of study design present opportunities to understand the trade-offs between effort and precision [@cappelle_estimating_2021].
But similarly, disagreements on the implementation of surveying methods have led to further explorations of how to conceptualise and simulate animal processes [@abolaffio_avoiding_2019].
...
There are a number of approaches for simulating animal movement including (but not limited to) the following:
...
The *abmR* package shows how Ornstein-Uhlenbeck scales up to simulate large animal movements [@gochanour_span_2023].
A key aspect of animal movement they simulate is correlated speeds and central tendency (i.e., home range).

2. hidden markov models, exemplified by the `simData` function from the *moveHMM* package [@Michelot2016].
...
3. agent-based modelling, is a bottom-up approach where simulations of an agent (e.g., an animal) are based upon a number of rules [@Tang2010], and through repeatedly following those rules a complex output emerges.
In the model we present here, this output represents the movement pathways in a landscape.
...
A new independently developed approach to simulating animal movement will provide additional routes to test the robustness of movement analyses.
The simulation is primarily set-up to create a foundation for exploring whether different habitat selection analyses result in the same/similar conclusions [more thoroughly explored, using the example simulated species here, in @marshall_habitat_2024; @marshall_multiverse_2024].
...
The animal has three behavioural states, broadly defined here as state 0 - sheltering (or resting), state 1 - exploring, and state 2 - foraging, that broadly define its movement characteristics and goals.
...
The movement characteristics are defined by two distributions: a Gamma distribution that step lengths are drawn from, and a Von-Mises distribution that turn angles are drawn from.
...
These probabilities can be kept high to introduce autocorrelation in the behavioural state; a high autocorrelation is required if we are modelling time steps as one minute.
For the included example, we select probabilities that allow the animal to express all behavioural state reasonably evenly and allow behavioural cycles to be easily visible in the resulting simulated movement data.

``` {r transMatrixDiagram, echo=FALSE, eval=TRUE, fig.align='center', fig.width=5, fig.height=5, out.width="80%", fig.cap="Transition probabilities between the three behavioural states in abmAnimalMovement package example. Remain values indicate the probabilities of the animal remaining in the same behavioural state. Values correspond directly to those in the transition matrix used as a simulation input."}
...
This desired destination remains constant for the duration the animal remains in the foraging state; subsequent times the animal enters the foraging state a new destination is selected.
While in the foraging state, this destination attraction impacts the movement choices made at each time step, with the animal more likely to choose (and therefore move to) options closer to the foraging destination.
Therefore, foraging destination choice operates on a different time frame to the movement and allows movements through low-quality foraging areas to reach a high-quality destination.
...
The examples cover a range of movement capacities, site fidelities, and resting/sheltering patterns.
...
Therefore, we are only interested in whether the simulated data return the intended broad traits we are attempting to simulate for each example species (e.g., avoidance, site fidelity, expected step lengths).
We show one example via a more detailed walk-through (badger), and the other example parametrisations are included as use cases at the end of the manuscript (vulture, king cobra).
...
<!-- shelter -->
Badgers occupy setts, in our example two home/shelter sites, to where they routinely return [@kowalczyk_daily_2006; @feore_habitat_1999].
...
<!-- movement -->
We can draw on statements regarding maximum distance travelled in a night from papers such as @rosalino_activity_2005, @kowalczyk_daily_2006, and @loureiro_path_2007 to approximate how distant foraging locations could be from a sett, while also confirming the nocturnal activity cycle for the badger.
<!-- cycles -->
...
Badgers occupying temperate areas are impacted by seasonal shifts that modify daylight hours and available resources [@magowan_dead-reckoning_2022; @rosalino_activity_2005].
<!-- avoidance -->
...
1. Exhibits site fidelity via the use of two shelter sites [@kowalczyk_daily_2006; @feore_habitat_1999]

2. Movement speed approximated by summary statistics from previous studies [@kowalczyk_daily_2006; @rosalino_activity_2005], while constrained by terrestrial environment and territoriality [@kelly_extra_2020; @feore_habitat_1999]

3. An 8-12 hour activity cycle, that shifts over the year [@magowan_dead-reckoning_2022; @rosalino_activity_2005]
...
library(raster) # environmental matrix generation
library(NLMR) # environmental matrix generation
```
...
We used the *raster* `r paste0("v.", packageVersion("raster"))` [@R-raster], *sp* `r paste0("v.", packageVersion("sp"))` [@sp2013; @sp2005; @R-sp], and *NLMR* `r paste0("v.", packageVersion("NLMR"))` [@NLMR2018; @R-NLMR] for generating environmental matrices. *NLMR* is no long available on CRAN but still accessible here https://github.com/ropensci/NLMR, and installed using `remotes::install_github("ropensci/NLMR")`.
...
Depending on the behavioural state, the weighing of which matrix/layer to use will change (e.g., when in a sheltering behavioural state the shelter site quality layer is used).

For most applications, a landscape would be known *a priori*, but for this demonstration and testing of methods, we use a selection of randomly generated landscape matrices (Figure \@ref(fig:BADGERlayersFigure)), using the *NLMR* package (neutral landscape models).
...
Once generated we supply the core simulation function (`abm_simulate`) with each of these layers via the `shelteringMatrix`, `foragingMatrix`, and `movementMatrix` arguments (Table \@ref(tab:arguementTable)).

```{r rfOptions, echo = FALSE, eval=FALSE, warning=FALSE, message=FALSE, comment=FALSE}
RandomFields::RFoptions(install="no")
```

```{r landscapeLayers, echo=FALSE, warning=FALSE, message=FALSE, comment=FALSE, eval=FALSE, error=FALSE, results=FALSE}
...
```{r landscapeLayersEgData, echo=FALSE, warning=FALSE, message=FALSE, comment=FALSE, eval=TRUE, error=FALSE, results=FALSE}
...
We predefine a suite of parameters describing the movement and behaviour characteristics of the animal (e.g., badger) based on summary statistics and general statements from previous research [@kowalczyk_daily_2006; @feore_habitat_1999; @rosalino_activity_2005; @kelly_extra_2020; @magowan_dead-reckoning_2022].
We store the parameters as objects we can later input into the `abm_simulate` function.
...
VULTURE_shelterSize <- 5 # shelter site radius to reduce movements
```

```{r BADGERlayersFigure, eval=TRUE, echo=FALSE, warnings=FALSE, fig.align='center', fig.width=7, fig.height=4, fig.cap="The three resulting landscape layers to be input into the simulation for the badger example: shelter quality, foraging resources, movement ease. The points marked with S indicate the generated shelter sites randomly sampled from the underlying shelter quality matrix. X and Y axes cell numbers; in the case of the badger example cell is 5 m by 5 m."}
...
For the turn angles, we require the mean ($\mu$) and concentration ($\kappa$) to define the Von Mises distributions and provide these via the `mu_angle` and `k_angle` arguments.

<!-- destination -->
The perceptual range (implemented here as destination range) is how far the the badger can "see" when deciding where to forage and is set in a similar fashion.
Where `destinationRange` provides the shape (k) and scale ($\theta$) for the Gamma distribution describing distance of possible foraging locations, and `destinationDirection` provides the mean ($\mu$) and concentration ($\kappa$), the Von Mises distribution describes the angle at which those locations can fall in relation to the previous direction of movement. (Figure \@ref(fig:BADGERmoveCharFigure)).
...
Values supplied to `destinationModifier` apply a linear multiplicative effect to all the weightings.
...
# behaviour state
VULTURE_k_step <- c(2, 2.2*60, 1.5*60) # Gamma shape
VULTURE_s_step <- c(40, 1.2, 1) # Gamma scale
VULTURE_mu_angle <- c(0, 0, 0) # Von Mises mean
VULTURE_k_angle <- c(0.6, 0.99, 0.6) # Von Mises concentration
...
VULTURE_rescale <- 20 # each cell of the environmental matrix is 20x20
```
```{r KINGCOBRAsettingMoveDes, eval=TRUE, echo=FALSE}

# parameters defining Gamma (step length) and Von Mises (turn angle) for each
# behaviour state
KINGCOBRA_k_step <- c(30, 40, 20) # Gamma shape
KINGCOBRA_s_step <- c(0.75, 1.2, 1.75) # Gamma scale
KINGCOBRA_mu_angle <- c(0, 0, 0) # Von Mises mean
KINGCOBRA_k_angle <- c(0.6, 0.99, 0.6) # Von Mises concentration
...
KINGCOBRA_rescale <- 10 # each cell of the environmental matrix is 10x10
```

<!-- avoidance -->
The territoriality is not simulated directly via other individuals. 
Instead we approximate it by supplying a number of point locations the badger will avoid.
...
For `rest_Cycle`, we will store a vector of length 4, with values defining the amplitude, midline, offset, and frequency in that order (Frequency is supplied in hours, i.e., 60 times the time step in this case).
...
To add a seasonal cycle, we use `additional_Cycles`.
...
# the diel sheltering/active cycle: amplitude, midline, offset, frequency
...
KINGCOBRA_shelterLocs <- data.frame(
  "x" = sampledShelters[,2],
  "y" = sampledShelters[,1])
KINGCOBRA_shelterSize <- 10 # specifying a larger shelter site radius
```
...
Equally `timesteps` can be thought of as the number of movement choices simulated.
...
We can then call all our settings describing badger behaviour and movement settings we stored as objects previously, and run the simulation using `abm_simulate` (Table \@ref(tab:arguementTable)).

``` {r runSimulationsSHOW, include=FALSE, eval=FALSE}
simSteps <- 24*60 *365
...
``` {r runSimulationsRUN, include=FALSE, eval=FALSE, message=FALSE}
simSteps <- 24*60 *365
des_options <- 10
options <- 12
...
# save sim data for repo as pre-baked example
for(species in vecSpecies){
  
  write.csv(x = simResultsList[[species]]$locations,
            file = here::here("notebook",
                              "manuscript",
                              "eg_simdata",
                              paste0("eg_simdata_", species, "_locations.csv")),
            row.names = FALSE)
}
...
```{r runSimulationsEgData, echo=FALSE, warning=FALSE, message=FALSE, comment=FALSE, eval=TRUE, error=FALSE, results=FALSE}
simSteps <- 24*60 *365
des_options <- 10
options <- 12

vecSpecies <- c("BADGER", "VULTURE", "KINGCOBRA")
simResultsList <- readRDS(here::here("notebook",
                                     "manuscript",
                                     "eg_simdata",
                                     paste0("eg_simdata_completelist.RDS")))

arguementTable <- read.csv(file = here::here("notebook",
                           "manuscript",
                           "eg_simdata", "argument_table.csv"))
```

```{r arguementTable, echo=FALSE, message=FALSE, warning=FALSE}
knitr::kable(arguementTable, booktabs = TRUE, caption = "Arguments fed into main \\texttt{abm\\_simulate} function. Arguments are the terms used in the function, the inputs are the R objects created during this example, the descriptions provide a brief overview of what the argument is configuring.") %>% 
  kableExtra::kable_styling(full_width = FALSE, bootstrap_options = "striped", font_size = 10, position = "center") %>%
...
The simulation function (`abm_simulate`) outputs a list containing: (1) a dataframe of realised simulated movements, (2) a dataframe of options the animal had available, and (3) a list of the inputs used to simulate the movement.
The realised movement dataframe (`locations`) describes all simulated locations the animal occupied, step length information, behavioural state, and current point of attraction, where each row is equal to a time step.
...
n <- 1000
moveCharPlotList <- vector("list", length = length(vecSpecies))
...
for(species in vecSpecies){
  ...
  if(species == "BADGER"){
    ...
    k_step <- VULTURE_k_step
    s_step <- VULTURE_s_step
    destinationRange <- VULTURE_destinationRange
  } else if(species == "KINGCOBRA"){
    k_step <- KINGCOBRA_k_step
    s_step <- KINGCOBRA_s_step
    destinationRange <- KINGCOBRA_destinationRange
  }
  
  paraStepData <- data.frame(
    "var" = factor(c(
    ...
    behave = c(rep(0, n),
               ...
               rep(2, n)),
    "value" =
      c(rgamma(n, shape = k_step[1], scale = s_step[1]),
        rgamma(n, shape = k_step[2], scale = s_step[2]),
        rgamma(n, shape = k_step[3], scale = s_step[3])))
  
  paraDestData <- data.frame(
    "var" = rep("Destination range", n),
    "value" = rgamma(n, shape = destinationRange[1],
                     scale = destinationRange[2]))
  ...
  for(beh in 0:2){
    ...
    zeroPlot <- ggplot(stepTextData %>% 
      ...
      scale_fill_manual(
        values = c("Stationary" = palette[names(palette) == beh][[1]], "grey75")) +
      theme_void() +
    ...
    stepPlot <- stepData %>% 
      ...
      geom_density(data = paraStepData %>% 
                     filter(behave == beh),
                   aes(x = value),
                   fill = NA,
                   colour = palette[names(palette) == beh], alpha = 0.5,
                   size = 1.15) +
    ...
    b_destPlot <- paraDestData %>% 
      ggplot() +
      geom_density(aes(x = value), alpha = 0.5,
                   colour = NA, fill = "grey15") +
...
for(species in vecSpecies){
  ...
  if(species == "BADGER"){
    tempPlot <- tempPlot +
      ...
      ggplot2::theme(axis.text.y = ggplot2::element_text(),
                     axis.title.y.left = ggplot2::element_text(size = 4,
                                                               angle = 0,
                                                               face = 2,
                                                               vjust = 1,
                                                               hjust = 1))
  ...
  if(species == "KINGCOBRA"){
    tempPlot <- tempPlot +
      labs(x = "Cell number x") +
      ggplot2::theme(axis.title.x = ggplot2::element_text(size = 4,
                                                          angle = 0,
                                                          face = 2,
                                                          vjust = 1,
                                                          hjust = 1))
...
for(species in vecSpecies){
  ...
  shelterPlot <- behPlotList[["0 - Shelter"]] +
                    theme(axis.text.x = element_blank(),
                          axis.title.x = element_blank())
  explorePlot <- behPlotList[["1 - Explore"]] +
       theme(axis.text.x = element_blank(),
             plot.title = element_blank(),
             axis.title.x = element_blank())
  foragePlot <- behPlotList[["2 - Forage"]] +
       theme(plot.title = element_blank())
  
  if(species == "BADGER"){
    shelterPlot <- shelterPlot +
      labs(y = "Cell\nnumber y") +
      ggplot2::theme(axis.text.y = ggplot2::element_text(),
                     axis.title.y.left = ggplot2::element_text(size = 4,
                                                               angle = 0,
...
Some of the differences are a result of the different behavioural transition matrices provided to simulate the two species, where both had different baseline probabilities of shifting between behavioural states.
...
```{r cycleSeasonalFigure, echo=FALSE, eval=TRUE, message=FALSE, warning=FALSE, fig.align='center', fig.width=6, fig.height=4, fig.cap="The percentage of time steps spent in state 0 - sheltering per day, and how it varies over the entire simulated year."}
...
While *ideal* simulations often accompany new analysis methods and provide superb validation for the method in question, reaffirming the method's robustness and pushing them to new limits with a messier, more stochastic, simulation approach could greatly strengthen our confidence in results.
The *abmAnimalMovement* package provides an independent route to test new methods, which covers a range of interacting movement features, not necessarily directly tied to a single analytical method.
For example, we have used this simulation to explore the impacts of analysis choice on researcher's ability to extract habitat selection estimates from movement data [@marshall_habitat_2024; @marshall_multiverse_2024].
...
For animals demonstrating more behavioural states, the *abmAnimalMovement* package would need considerable extension to be useful; the same for migratory species, especially those travelling long distances into mutually exclusive areas.
...
Via the exploration of different parametrisations, researchers can help build a picture of the study and analysis methods best suited for their questions, with the opportunity to test those analyses on synthetic simulated data.
...
Additionally, such synthetic datasets could be integrated into preregistration as a means of demonstrating the validity of an analysis plan prior to undertaking a study, or as post hoc exploration of sampling uncertainty [@Silva_movedesign_2023].

Producing a range of simulated datasets that cover alternate scenarios may present researchers with opportunities to test real data against a null model.
...
1. Dynamic state 0. While state 0 and the steady state of the attraction locations is key to simulating range stability (i.e., home range), there may be scenarios where this stability is not desired.
...
Primarily, we created these further examples for use in @marshall_habitat_2024 and @marshall_multiverse_2024, to broaden the generalisablity of the findings explored in those papers.
...
Vultures can also move greater distances more rapidly [@hribsek_first_2021], resulting in a more variable and distribution of step lengths [@garcia-jimenez_drivers_2018; @margalida_spatial_2016; @subedi_spatial_2020] (Figure \@ref(fig:VULTUREmoveCharFigure)).
...
Such shelter sites could be predefined; for example, if shelter sites were known *a priori* discovered via the capture and tagging of animals, which in the case of birds is more likely.
...
Their moments have been seen to follow carcasses, creating starkly contrasting areas where vultures will and will not travel [@arrondo_invisible_2018] (Figure \@ref(fig:VULTURElayersFigure)).
...
1. Medium site fidelity via the use of multiple roosting/sheltering sites [@bracis_revisitation_2018]

2. Movement speed approximated by summary statistics from previous studies [@hribsek_first_2021; @garcia-jimenez_drivers_2018; @margalida_spatial_2016; @subedi_spatial_2020], with minimal landscape derived resistance
...
Finally, king cobra movement characteristics will be dramatically reduced compared to the vulture's, but with similar shape to the badger with greater variability (Figure \@ref(fig:KINGCOBRAmoveCharFigure)) as king cobras are known to range over large areas [@Silva2018; @Marshall2018; @marshall_no_2020].

We can broadly summarise the king cobra ecology we want to parametrise as follows:

1. Lower site fidelity via the use of many shelter sites [@Marshall2018; @marshall_no_2020]

2. Movement speed approximated by summary statistics from previous studies [@Silva2018; @Marshall2018; @marshall_no_2020], with examples of very high landscape derived resistance [@jones_how_2022; @Marshall2018b]

3. A 8-12 hour activity cycle, with approximately weekly forage-digest cycle, and weak seasonality [@dsouza_space_2021; @smith_native_2021; @marshall_no_2020; @Siers2018]

### Simulation inputs
```{r palettePreview, echo=FALSE, eval=FALSE, fig.align='center', fig.width=7, fig.height=2.5, fig.cap="Universal colour key for behavioural states 0 - sheltering, 1 - foraging, 2 - exploring; and example species. Name assigned to the colour in bold, and the hex code in italics below."}
names(palette) <- c("KING\nCOBRA", "VULTURE", "BADGER", "2", "1", "0")
data.frame(
...
The vulture parametrisation largely matches the badger.
We specified a number of predefined resting/shelter sites, assuming that bird bio-tagging is more likely to occur at known sites (i.e., nests or roosts).
...
We also set a larger rescale value to ensure the vulture has a larger landscape to operate across.
...
The diel cycle is largely similar to the other examples, but we also define an approximately weekly cycle to cover the forage-digestion cycle as well as a weak seasonal cycle.
...
```{r VULTUREmoveCharFigure, echo=FALSE, eval=TRUE, warning=FALSE, fig.align='center', fig.width=7, fig.height=7, fig.cap="The distribution of step lengths used during the vulture simulation example. Parametrising distributions are shown as solid areas using the same shape and scale parameters used as input into the simulation function. The lower plot shows the distribution used to generate potential foraging destinations. The realised resulting simulated step lengths (scaled to the input units) are shown by the solid lines. The realised resulting turn angles are shown in left hand sub-plots. Inset pie charts show the number of step lengths that were below the shelter site size. Note that x axis is not consistent between the three plots."}

moveCharPlotList[["VULTURE"]]

```

```{r KINGCOBRAmoveCharFigure, echo=FALSE, eval=TRUE, warning=FALSE, fig.align='center', fig.width=7, fig.height=7, fig.cap="The distribution of step lengths used during the king cobra simulation example. Parametrising distributions are shown as solid areas using the same shape and scale parameters used as input into the simulation function. The lower plot shows the distribution used to generate potential foraging destinations. The realised resulting simulated step lengths (scaled to the input units) are shown by the solid lines. The realised resulting turn angles are shown in left hand sub-plots. Inset pie charts show the number of step lengths that were below the shelter site size. Note that x axis is not consistent between the three plots."}
...
The example badger, vulture, and king cobra data simulated for this manuscript are available on Zenodo: "Simulated data from abmAnimalMovement: An R package for simulating animal movement using an agent-based model" https://doi.org/10.5281/zenodo.6992495. Files include:
...
Source code to reproduce the results and manuscript are available alongside the software, archived at: https://doi.org/10.5281/zenodo.6951937.

The parameters used to generate the examples are within: `notebook/manuscript/Agent-based_model_walkthrough.Rmd`.

# Software availability

Examples of the simulation output can be found at https://doi.org/10.5281/zenodo.6992495, alongside the required landscape files to recreate those simulations.

Archived versions of the software are available on Zenodo: "BenMMarshall/abmAnimalMovement" https://doi.org/10.5281/zenodo.6951937.
...
A minimally dependent example of the simulation code is available in the package vignette.
